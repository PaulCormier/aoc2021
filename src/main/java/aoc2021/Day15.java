...
public class Day15 {
    ...
    public static void main(String[] args) {
        ...
        log.info("The length of the path with the lowest risk level in the bigger test data is: {}.", part2(testMap));

        log.setLevel(Level.DEBUG);
        //        log.setLevel(Level.INFO);

        log.info("The length of the path with the lowest risk level in the bigger real data is: {}.", part2(map));
    }
    ...
    private static int part2(final int[][] map) {
        ...
        int[][] bigMap = Stream.of(Arrays.copyOf(map, map.length * 5))
                               .map(r -> Arrays.copyOf(ObjectUtils.defaultIfNull(r, new int[map[0].length]), map[0].length * 5))
                               .collect(Collectors.toList())
            ...
            for (int gridCol = 0; gridCol < 5; gridCol++) {
                for (int row = 0; row < map.length; row++) {
                    for (int col = 0; col < map[row].length; col++) {
                        bigMap[gridRow * map.length + row][gridCol * map[row].length + col] = newValues[map[row][col] + gridRow + gridCol];
                    }
                }
            }
        }
        ...
        log.setLevel(Level.INFO);
        return traverseFrom(0, 0, bigMap, new int[bigMap.length][bigMap[0].length]) - bigMap[0][0];
    }
